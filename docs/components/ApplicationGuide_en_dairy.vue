  ...
  <div class="app-container">
    ...
    <div v-else class="main-view">
      ...
      <div class="debug-info">
        ...
        <p>dairy Records: {{ filteredItems.length }}</p>
      </div>
...
const selectedSubcategory = ref('dairy') // 固定為 dairy
...
// 只顯示 dairy 類別的 subclasses
const subclasses = computed(() => {
  const dairyItems = data.value.filter(item => 
    item.subcategory && item.subcategory.toLowerCase() === 'dairy'
  )
  return [...new Set(dairyItems.map((item) => item.subclass))].sort()
})

// 只顯示 dairy 類別的 actionTypes
const actionTypes = computed(() => {
  const dairyItems = data.value.filter(item => 
    item.subcategory && item.subcategory.toLowerCase() === 'dairy'
  )
  return [...new Set(dairyItems.map((item) => item.actionType))].sort()
})
...
const filteredItems = computed(() => {
  return data.value.filter((item) => {
    ...
    // 強制只顯示 dairy 類別
    if (subcategory.toLowerCase() !== 'dairy') {
      return false
    }
