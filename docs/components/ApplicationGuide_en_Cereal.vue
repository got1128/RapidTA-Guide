  ...
  <div class="app-container">
    ...
    <div v-else class="main-view">
      ...
      <div class="debug-info">
        ...
        <p>cereal Records: {{ filteredItems.length }}</p>
      </div>
...
const selectedSubcategory = ref('cereal') // 固定為 cereal
...
// 只顯示 cereal 類別的 subclasses
const subclasses = computed(() => {
  const cerealItems = data.value.filter(item => 
    item.subcategory && item.subcategory.toLowerCase() === 'cereal'
  )
  return [...new Set(cerealItems.map((item) => item.subclass))].sort()
})

// 只顯示 cereal 類別的 actionTypes
const actionTypes = computed(() => {
  const cerealItems = data.value.filter(item => 
    item.subcategory && item.subcategory.toLowerCase() === 'cereal'
  )
  return [...new Set(cerealItems.map((item) => item.actionType))].sort()
})
...
const filteredItems = computed(() => {
  return data.value.filter((item) => {
    ...
    // 強制只顯示 cereal 類別
    if (subcategory.toLowerCase() !== 'cereal') {
      return false
    }
